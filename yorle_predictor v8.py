...
st.set_page_config(
    page_title="Cash winPredictor v7.0",
    page_icon="📢",
    layout="wide"
)

st.title("📢 Cash winPredictor v7.0 - Estrategia de Persistencia (Eco)")
...
def build_affinity_map(_df):
    affinity_map = {}
    df_sorted = _df.sort_values(by=['fecha', 'franja_order']).reset_index()
...
def generar_prediccion_afinidad(df, params):
    ...
    last_num = df.sort_values(by=['fecha', 'franja_order']).iloc[-1]['numero']
...
# <--- NUEVO: Función para la "Estrategia de Persistencia (Eco)" --->
...
def render_strategy_parameters(strategy_name, key_prefix):
    ...
    if "Persistencia" in strategy_name:
        ...
        params['umbral_confianza'] = st.sidebar.number_input("Umbral de Confianza (mín. apariciones)", 1, 10, 2, 1, key=f"{key_prefix}_uc_afinidad")
...
def modulo_prediccion():
    ...
    if not mejores_candidatos:
        st.error("La estrategia seleccionada no pudo generar candidatos con los parámetros actuales. Prueba a ajustar los parámetros o usa más datos históricos.")
        return
    ...
    if st.button("▶️ Ejecutar Backtesting con Parámetros Actuales"):
        with st.spinner(f"🧠 Simulado la '{strategy_name_bt}'... por favor espera."):
            ...
            for fecha_str in sorted(df_test["fecha"].unique()):
                df_historico_real = df[df['fecha'] < fecha_str]
                ...
                for _, row in dia_df_real.iterrows():
                    real = int(row["numero"])
                    acierto = real in prediccion_dia_base
                    ...
                    resultados_bt.append({
                        "fecha": fecha_str, "franja": row['franja'], "predicho": ", ".join(map(str, sorted(prediccion_dia_base))), 
                        "real": real, "acierto": acierto, "sorteos_espera": sorteos_espera, "racha_general": racha_general
                    })
                    
                    if "Doble Estrategia" in strategy_name_bt and prediccion_dia_base:
                        if acierto: pesos_bt[real] = max(0, pesos_bt.get(real, 0) - 1)
                        else: pesos_bt[prediccion_dia_base[0]] = pesos_bt.get(prediccion_dia_base[0], 0) + 1
