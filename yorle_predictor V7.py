...
st.set_page_config(
    page_title="Cash winPredictor v6.1",
    page_icon="🤝",
    layout="wide"
)

st.title("🤝 Cash winPredictor v6.1 - Estrategia de Afinidad")
...
# <--- NUEVO: Funciones para la "Estrategia de Afinidad" --->
@st.cache_data
...
def generar_prediccion_afinidad(df, params):
    ...
    if last_num not in affinity_map:
        return []
...
def modulo_prediccion():
    ...
    strategy_options = ["Estrategia de Afinidad 🤝", "Estrategia del Detective 🕵️", "Patrones de Corto Plazo 📈", "Doble Estrategia 🔁"]
    ...
    if not mejores_candidatos:
        st.error("La estrategia seleccionada no pudo generar candidatos con los parámetros actuales.")
        return
...
def modulo_backtesting():
    ...
    strategy_options_bt = ["Estrategia de Afinidad 🤝", "Estrategia del Detective 🕵️", "Patrones de Corto Plazo 📈", "Doble Estrategia 🔁"]
    ...
    if st.button("▶️ Ejecutar Backtesting con Parámetros Actuales"):
        with st.spinner(f"🧠 Simulado la '{strategy_name_bt}'... por favor espera."):
            df_test = df_sorted[(df_sorted["fecha"] >= fecha_inicio) & (df_sorted["fecha"] <= fecha_fin)]
            resultados_bt = []
            
            todos_los_numeros_historicos = df_sorted['numero'].unique()
            ...
            for index, row in df_test.iterrows():
                df_historico_real = df_sorted.iloc[:row.name]
                if df_historico_real.empty: continue
                
                candidatos_dia = []
                if "Afinidad" in strategy_name_bt:
                    last_num = df_historico_real.iloc[-1]['numero']
                    affinity_map = build_affinity_map(df_historico_real.copy())
                    if last_num in affinity_map:
                        followers = affinity_map[last_num]
                        confiables = {num: count for num, count in followers.items() if count >= params_bt.get('umbral_confianza', 2)}
                        if confiables:
                            candidatos_dia = sorted(confiables, key=confiables.get, reverse=True)[:params_bt.get('numero_candidatos', 5)]
                elif "Detective" in strategy_name_bt:
                    candidatos_dia = generar_prediccion_detective(df_historico_real.copy(), row['fecha'], params_bt)
                    ...
                    candidatos_dia = generar_prediccion_corto_plazo(df_historico_real.copy(), row['fecha'], params_bt['ventana_dias'], params_bt['numero_candidatos'], params_bt['tipo_ponderacion'])
                
                if not candidatos_dia or len(candidatos_dia) < 3: continue
                
                prediccion_triple = candidatos_dia[:3]
                real = int(row["numero"])
                ...
                resultados_bt.append({
                    "fecha": row['fecha'], "franja": row['franja'], "predicho": ", ".join(map(str, sorted(prediccion_triple))), 
