...
st.set_page_config(
    page_title="Cash winPredictor v7.2",
    page_icon="📢",
    layout="wide"
)

st.title("📢 Cash winPredictor v7.2 - Código Limpio y Estable")
...
def generar_prediccion_corto_plazo(df_historico, fecha_actual_str, ventana_dias, numero_candidatos, tipo_ponderacion):
    ...
    return puntuaciones.sort_values(ascending=False).head(numero_candidatos).index.tolist()
...
def generar_prediccion_detective(df, fecha_str, params):
    ...
    df_reciente = df[pd.to_datetime(df['fecha']) < fecha_actual].copy() # Usamos .copy() para seguridad
...
def build_affinity_map(_df):
    ...
    df_copy = _df.copy()
    df_copy['franja_order'] = df_copy['franja'].map(franja_map)
    df_sorted = df_copy.sort_values(by=['fecha', 'franja_order']).reset_index()
    for i in range(len(df_sorted) - 1):
        current_num = df_sorted.loc[i, 'numero']
        next_num = df_sorted.loc[i + 1, 'numero']
        if current_num not in affinity_map:
            affinity_map[current_num] = []
        affinity_map[current_num].append(next_num)
    for num, next_nums in affinity_map.items():
        affinity_map[num] = Counter(next_nums)
    return affinity_map

def generar_prediccion_afinidad(df, params):
    if len(df) < 2: return []
    affinity_map = build_affinity_map(df.copy())
    franja_map = {franja: i for i, franja in enumerate(franjas)}
    df_copy = df.copy()
    df_copy['franja_order'] = df_copy['franja'].map(franja_map)
    last_num = df_copy.sort_values(by=['fecha', 'franja_order']).iloc[-1]['numero']
...
def generar_prediccion_persistencia(df, fecha_actual_str, params):
    try:
        fecha_actual = pd.to_datetime(fecha_actual_str).date()
        fecha_base = fecha_actual - timedelta(days=params.get('retraso_dias', 1))
        fecha_base_str = fecha_base.strftime('%Y-%m-%d')
    except Exception:
        return []

    numeros_del_dia_base = df[df['fecha'] == fecha_base_str]['numero'].unique().tolist()
    return numeros_del_dia_base
...
def render_strategy_parameters(strategy_name, key_prefix):
    ...
    if "Persistencia" in strategy_name:
        params['retraso_dias'] = st.sidebar.slider("Días de Retraso (Eco)", 1, 7, 1, key=f"{key_prefix}_rd_pers")
...
def modulo_prediccion():
    ...
    strategy_options = ["Estrategia de Persistencia (Eco) 📢", "Estrategia de Afinidad 🤝", "Estrategia del Detective 🕵️", "Patrones de Corto Plazo 📈", "Doble Estrategia 🔁"]
    ...
    if not st.session_state.resultados or len(st.session_state.resultados) < 10:
    ...
    if "Persistencia" in strategy_name:
        st.markdown(f"📢 **Estrategia de Persistencia (Eco)** — Apostando por la repetición de los números de hace `{params['retraso_dias']}` día(s).")
        hoy_str = datetime.today().strftime("%Y-%m-%d")
        mejores_candidatos = generar_prediccion_persistencia(df.copy(), hoy_str, params)
    elif "Afinidad" in strategy_name:
        st.markdown(f"🤝 **Estrategia de Afinidad** — Buscando los 'amigos' del último número ganador.")
        mejores_candidatos = generar_prediccion_afinidad(df.copy(), params)
    elif "Detective" in strategy_name:
    ...
    if not mejores_candidatos:
        st.error("La estrategia seleccionada no pudo generar candidatos. Prueba a ajustar los parámetros.")
        return
        
    if len(mejores_candidatos) < 3 and "Persistencia" not in strategy_name:
    ...
    for i, franja in enumerate(franjas):
        prediccion_base = mejores_candidatos[:3] if "Persistencia" not in strategy_name else mejores_candidatos
        prediccion_final = prediccion_base if "Persistencia" in strategy_name else list(set([mejores_candidatos[(i + j) % len(mejores_candidatos)] for j in range(3)]))
        numeros_str = ", ".join(map(str, sorted(prediccion_final)))
...
def modulo_backtesting():
    ...
    strategy_options_bt = ["Estrategia de Persistencia (Eco) 📢", "Estrategia de Afinidad 🤝", "Estrategia del Detective 🕵️", "Patrones de Corto Plazo 📈", "Doble Estrategia 🔁"]
    ...
    if st.button("▶️ Ejecutar Backtesting con Parámetros Actuales"):
        with st.spinner(f"🧠 Simulado la '{strategy_name_bt}'... por favor espera."):
            ...
            for fecha_str in sorted(df_test["fecha"].unique()):
                # <--- CORRECCIÓN CLAVE: Usamos .copy() para evitar la advertencia --->
                df_historico_real = df[df['fecha'] < fecha_str].copy()
                if df_historico_real.empty: continue
                
                candidatos_dia = []
                if "Persistencia" in strategy_name_bt:
                    candidatos_dia = generar_prediccion_persistencia(df.copy(), fecha_str, params_bt)
                elif "Afinidad" in strategy_name_bt:
                    # La función de afinidad necesita el orden de las franjas
                    franja_map = {franja: i for i, franja in enumerate(franjas)}
                    df_historico_real['franja_order'] = df_historico_real['franja'].map(franja_map)
                    candidatos_dia = generar_prediccion_afinidad(df_historico_real.copy(), params_bt)
                elif "Detective" in strategy_name_bt:
                    candidatos_dia = generar_prediccion_detective(df_historico_real.copy(), fecha_str, params_bt)
                elif "Doble Estrategia" in strategy_name_bt:
                    numeros_en_historico = df_historico_real['numero'].unique()
                    rotaciones_dia = [n for n in numeros_en_historico if (rot := calcular_rotacion(df_historico_real, n)) is not None and rot <= params_bt['umbral_rotacion']]
                    ...
                    candidatos_dia = generar_prediccion_corto_plazo(df_historico_real.copy(), fecha_str, params_bt['ventana_dias'], params_bt['numero_candidatos'], params_bt['tipo_ponderacion'])
                
                if not candidatos_dia: continue

                prediccion_dia_base = candidatos_dia

                dia_df_real = df_test[df_test["fecha"] == fecha_str]
                for _, row in dia_df_real.iterrows():
                    real = int(row["numero"])
                    prediccion_final = prediccion_dia_base if "Persistencia" in strategy_name_bt else prediccion_dia_base[:3]
                    acierto = real in prediccion_final
                    ...
                    resultados_bt.append({
                        "fecha": fecha_str, "franja": row['franja'], "predicho": ", ".join(map(str, sorted(prediccion_final))), 
                        "real": real, "acierto": acierto, "sorteos_espera": sorteos_espera, "racha_general": racha_general
                    })
                    
                    if "Doble Estrategia" in strategy_name_bt and prediccion_final:
                        if acierto: pesos_bt[real] = max(0, pesos_bt.get(real, 0) - 1)
                        else: pesos_bt[prediccion_final[0]] = pesos_bt.get(prediccion_final[0], 0) + 1
